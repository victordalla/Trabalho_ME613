...
```{r, fig.cap="\\label{fig:dispersao}Gráficos da váriavel resposta (preço), eixo y, em função das variáveis explicativas, eixo x, indicadas nos rótulos."}
...
# , fig.cap="\\label{fig:cor}Gráfico da matriz de correlação das covariáveis área construída (are_cons), área do terreno (area_ter), idade, imposto e da variável resposta. Quanto mais vermelho, mais positiva a correlação; quanto mais azul, mais negativa."
GGally::ggcorr(df)
```
...
tidy(modelo_simples, conf.int = TRUE) %>% 
  ...
  knitr::kable(caption = "\\label{tab:modelo1}Resultados do Modelo 1.")
```
...
tidy(modelo_reduzido, conf.int = TRUE) %>% 
  select(-statistic) %>%
  mutate(p.value = format.pval(p.value, eps = 0.001, digits = 2)) %>%
  knitr::kable(caption = "\\label{tab:modelo2}Resultados do Nodelo 2.")
```
...
tidy(modelo_reduzido, conf.int = TRUE) %>%
  select(-statistic) %>%
  mutate(p.value = format.pval(p.value, eps = 0.001, digits = 2)) %>%
  knitr::kable(caption = "\\label{tab:modelo3}Resultados do Modelo 3.")
```
