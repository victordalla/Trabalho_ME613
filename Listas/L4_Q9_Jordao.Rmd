...
```{r}
wts <- 1/fitted(m3)^2
m4 <- lm(peso ~ poly(idade, 3, raw = T), data = df, weights = wts)
summary(m4)
```
...
```{r}
termplot(m4, data = df, se = T, partial.resid = T)
```
...
```{r}
wts <- 1/fitted(m2)^2
m5 <- lm(peso ~ poly(idade, 2, raw = T), data = df, weights = wts)
summary(m5)
```
...
```{r}
termplot(m5, data = df, se = T, partial.resid = T)
```
...
```{r}
wts <- 1/fitted(m1)^2
m6 <- lm(peso ~ idade, data = df, weights = wts)
summary(m6)
```
...
```{r}
termplot(m6, data = df, se = T, partial.resid = T)
```
...
# Modelo Com Transformação de Variável

```{r}
m7 <- lm(peso ~ log(idade), data = df)
summary(m7)
```

`r summary(m7) %>% tidy() %>% kable()`


```{r}
termplot(m7, data = df, se = T, partial.resid = T)
```
...
`r AIC(m1, m2, m3, m4, m5, m6, m7) %>% kable()`

### BIC

`r BIC(m1, m2, m3, m4, m5, m6, m7) %>% kable()`

### $R^2$

```{r}
rsq <- function(x) summary(x)$r.squared
r2 <- data.frame(R2 = c(rsq(m1), rsq(m2), rsq(m3), rsq(m4), rsq(m5),
                        rsq(m6), rsq(m7)))
rownames(r2) <- c("m1", "m2", "m3", "m4", "m5", "m6", "m7")
```
